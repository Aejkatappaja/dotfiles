...
wifi_up:subscribe("network_update", function(env)
	local up_color = (env.upload == "000 Bps") and colors.grey or colors.sora.rose
	local down_color = (env.download == "000 Bps") and colors.grey or colors.sora.cyan
	wifi_up:set({
		icon = { color = up_color },
		label = {
			string = env.upload,
	...
	sbar.exec("ipconfig getifaddr en1", function(ip)
		...
		wifi:set({
			icon = {
				...
				color = connected and colors.sora.steel or colors.sora.rose,
			},
		})
	end)
end)
